(function () {
  ...
  function updatePracticeClock() {
    ...
    if (remaining === 0 && !practiceSession.finishedAt) finishPracticePaper();
  }
  ...
  function renderCaseAnalysis(question) {
    ...
    form.addEventListener('submit', event => { event.preventDefault(); const missing = answers.filter(answer => !answer.rating.value); if (missing.length) { state.textContent = '请为每个分问选择自评状态。'; return; } submit.disabled = true; state.textContent = '已保存本次案例作答，可展开参考要点自评。'; const details = element('details', 'practice-explanation'); details.append(element('summary', '', '展开参考要点对照')); answers.forEach(answer => { const block = element('section', 'reference-points'); block.append(element('h4', '', `${answer.item.id} 参考要点`)); const list = element('ul', 'source-list'); answer.item.reference_points.forEach(point => list.append(element('li', '', point))); block.append(list); details.append(block); }); practiceReader.append(details); addExplanation(question); const record = element('button', 'button secondary', '记录案例复盘'); record.type = 'button'; record.addEventListener('click', () => { const incomplete = answers.filter(answer => answer.rating.value !== '已覆盖'); const response = answers.map(answer => `${answer.item.id}[${answer.rating.value}]：${answer.text.value.trim() || '未作答'}`).join(' | ').slice(0, 1200); const references = incomplete.flatMap(answer => answer.item.reference_points).join('、').slice(0, 600); recordPractice(question, { type: '案例', result: incomplete.length ? '部分完成' : '基本掌握', promptEvidence: response, hitKeywords: answers.filter(answer => answer.rating.value === '已覆盖').map(answer => answer.item.id).join('、'), missedKeywords: references, errorType: incomplete.length ? '要点覆盖不足' : '', rule: question.explanation || '' }); }); practiceReader.append(record); }); practiceReader.append(form); renderPracticeNavigation();
  }
  function renderEssay(question) {
    ...
    form.addEventListener('submit', event => { event.preventDefault(); const project = form.elements.namedItem('project').value.trim(); if (!project) { state.textContent = '请至少填写项目背景与本人职责。'; return; } submit.disabled = true; state.textContent = '论文提纲已保留在当前页面，可展开参考提纲并记录论文素材。'; const details = element('details', 'practice-explanation'); details.append(element('summary', '', '展开参考审题与提纲')); const req = element('ul', 'source-list'); question.requirements.forEach(item => req.append(element('li', '', item))); details.append(element('h4', '', '写作要求'), req); const outline = element('ul', 'source-list'); question.reference_outline.forEach(item => outline.append(element('li', '', item))); details.append(element('h4', '', '参考提纲'), outline); practiceReader.append(details); addExplanation(question); const record = element('button', 'button secondary', '记录论文素材'); record.type = 'button'; record.addEventListener('click', () => recordPractice(question, { type: '论文素材', result: '待复习', promptEvidence: `摘要：${form.elements.namedItem('abstract').value.trim()}\n项目背景：${project}`.slice(0, 1200), projectCode: form.elements.namedItem('project_code').value.trim().slice(0, 120), decision: form.elements.namedItem('solution').value.trim(), tradeoff: form.elements.namedItem('tradeoff').value.trim(), outcome: form.elements.namedItem('outcome').value.trim(), missedKeywords: form.elements.namedItem('body').value.trim(), rule: form.elements.namedItem('solution').value.trim() })); practiceReader.append(record); }); practiceReader.append(form); renderPracticeNavigation();
  }
  async function openPracticeQuestion(id) {
    practiceState.textContent = '正在加载已核对题目…';
    try { let question = practiceQuestionCache.get(id); if (!question) { const response = await fetch(`/api/practice/questions/${encodeURIComponent(id)}`, { headers: { Accept: 'application/json' } }); const payload = await response.json(); if (!response.ok) throw new Error(payload.error || '题目加载失败'); question = payload.question; practiceQuestionCache.set(id, question); } activePracticeQuestion = question; practiceState.textContent = `正在练习：${question.title}`; if (question.type === 'single_choice') renderSingleChoice(question); else if (question.type === 'case_analysis') renderCaseAnalysis(question); else renderEssay(question); renderPracticeList(); renderPracticeCard(); }
  ...
  function renderPracticeCard() {
    ...
    practiceSession.questionIds.forEach((id, index) => { const button = element('button', `practice-card-item ${practiceSession.answers[id] ? 'answered' : ''}`, String(index + 1)); button.type = 'button'; button.setAttribute('aria-label', `第 ${index + 1} 题${practiceSession.answers[id] ? '，已作答' : ''}`); button.onclick = () => openPracticeQuestion(id); card.append(button); });
  ...
  async function finishPracticePaper() {
    ...
    const questions = [];
    for (const id of practiceSession.questionIds) { let item = practiceQuestionCache.get(id); if (!item) { const response = await fetch(`/api/practice/questions/${encodeURIComponent(id)}`); const payload = await response.json(); if (!response.ok) { practiceSession.finishedAt = 0; practiceTicker = window.setInterval(updatePracticeClock, 1000); practiceState.textContent = payload.error || '交卷时加载题目失败'; return; } item = payload.question; practiceQuestionCache.set(id, item); } questions.push(item); }
    const summary = PracticeState.summarize(practiceSession, questions); practiceReader.replaceChildren();
    ...
    const weak = element('p', 'muted', summary.weakKnowledge.length ? `薄弱知识点：${summary.weakKnowledge.map(item => `${item.knowledgeId}（${item.count}）`).join('、')}` : '本次未识别出薄弱知识点。'); practiceReader.append(weak);
    summary.results.forEach(result => { const details = element('details', 'practice-explanation'); details.append(element('summary', '', `${result.correct ? '✓' : '✗'} ${result.title}｜你选 ${result.selected || '未答'}｜正确 ${result.answer}`)); const review = element('button', 'text-action', '打开本题解析'); review.type = 'button'; review.onclick = () => { openPracticeQuestion(result.id); }; details.append(review); practiceReader.append(details); });
    if (summary.results.length) { const record = element('button', 'button secondary', '记录本次整卷练习'); record.type = 'button'; record.onclick = () => { const first = questions[0]; recordPractice(first, { result: summary.correct === summary.total ? '正确' : '部分完成', errorType: summary.correct === summary.total ? '' : '整卷薄弱点', rule: `正确率 ${summary.accuracy}%`, missedKeywords: summary.weakKnowledge.map(item => item.knowledgeId).join('、') }); }; practiceReader.append(record); }
    renderPracticeCard(); updatePracticeClock();
  }
  function startPracticeSession() {
    ...
    practiceSession = PracticeState.createSession(ids, document.getElementById('practice-limit').value); activePracticeQuestion = null; practiceQuestionCache.clear(); clearInterval(practiceTicker); practiceTicker = window.setInterval(updatePracticeClock, 1000); updatePracticeClock(); renderPracticeCard(); renderPracticeList();
