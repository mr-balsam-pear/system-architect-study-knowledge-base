(function () {
  ...
  const practiceFilters = {
    ...
    subject: document.getElementById('practice-subject'), knowledge_id: document.getElementById('practice-knowledge'),
  };
  ...
  function practiceDuration() {
    ...
    return `${Math.max(1, Math.round((Date.now() - practiceStartedAt) / 60000))} 分钟`;
  }
  function recordPractice(question, context = {}) {
    ...
    field('result').value = context.result || '待复习'; field('duration').value = practiceDuration();
    field('error_type').value = context.errorType || ''; field('rule').value = context.rule || '';
    field('missed_keywords').value = context.missedKeywords || ''; field('tags').value = others ? `关联知识点：${others}` : '';
  }
  function addPracticeHeader(question, label) {
    practiceReader.replaceChildren(); practiceStartedAt = Date.now();
  ...
  function renderSingleChoice(question) {
    ...
    question.options.forEach(option => { const label = element('label', 'choice-option'); const radio = document.createElement('input'); radio.type = 'radio'; radio.name = 'practice-choice'; radio.value = option.label; label.append(radio, element('strong', '', option.label), document.createTextNode(` ${option.text}`)); form.append(label); });
    const state = element('p', 'muted'); state.setAttribute('aria-live', 'polite'); const submit = element('button', 'button', '提交本题'); submit.type = 'submit';
  ...
  function renderCaseAnalysis(question) {
    ...
    const form = element('form', 'subjective-form'); const answers = [];
    question.questions.forEach(item => { const section = element('section', 'case-question'); section.append(element('h4', '', `${item.id}（${item.score} 分）`), element('p', '', item.prompt)); const text = document.createElement('textarea'); text.name = `case-${item.id}`; text.placeholder = '按要点分条作答；提交后可对照参考要点自评。'; section.append(text); const rating = document.createElement('select'); rating.name = `rating-${item.id}`; [['','请选择自评'],['已覆盖','已覆盖'],['部分覆盖','部分覆盖'],['未覆盖','未覆盖']].forEach(([value,label]) => { const option = element('option', '', label); option.value = value; rating.append(option); }); section.append(rating); answers.push({ item, text, rating }); form.append(section); });
    ...
    form.addEventListener('submit', event => { event.preventDefault(); const missing = answers.filter(answer => !answer.rating.value); if (missing.length) { state.textContent = '请为每个分问选择自评状态。'; return; } submit.disabled = true; state.textContent = '已保存本次案例作答，可展开参考要点自评。'; const details = element('details', 'practice-explanation'); details.append(element('summary', '', '展开参考要点对照')); answers.forEach(answer => { const block = element('section', 'reference-points'); block.append(element('h4', '', `${answer.item.id} 参考要点`)); const list = element('ul', 'source-list'); answer.item.reference_points.forEach(point => list.append(element('li', '', point))); block.append(list); details.append(block); }); practiceReader.append(details); addExplanation(question); const record = element('button', 'button secondary', '记录案例复盘'); record.type = 'button'; record.addEventListener('click', () => { const missed = answers.filter(answer => answer.rating.value !== '已覆盖').map(answer => answer.item.id).join('、'); recordPractice(question, { type: '案例', result: missed ? '部分完成' : '基本掌握', missedKeywords: missed, errorType: missed ? '要点覆盖不足' : '', rule: question.explanation || '' }); }); practiceReader.append(record); }); practiceReader.append(form);
  }
  function renderEssay(question) {
    addPracticeHeader(question, '论文训练'); const form = element('form', 'subjective-form'); const fields = [['abstract','摘要'],['project','项目背景与本人职责'],['solution','架构方案'],['tradeoff','关键取舍'],['outcome','效果验证'],['body','正文 / 详细提纲']];
    fields.forEach(([name,label]) => { const wrap = element('label', 'essay-field', label); const area = document.createElement('textarea'); area.name = name; area.placeholder = `填写${label}`; wrap.append(area); form.append(wrap); }); const submit = element('button', 'button', '保存论文提纲'); submit.type = 'submit'; const state = element('p', 'muted'); state.setAttribute('aria-live', 'polite'); form.append(submit, state);
    form.addEventListener('submit', event => { event.preventDefault(); const project = form.elements.namedItem('project').value.trim(); if (!project) { state.textContent = '请至少填写项目背景与本人职责。'; return; } submit.disabled = true; state.textContent = '论文提纲已保留在当前页面，可展开参考提纲并记录论文素材。'; const details = element('details', 'practice-explanation'); details.append(element('summary', '', '展开参考审题与提纲')); const req = element('ul', 'source-list'); question.requirements.forEach(item => req.append(element('li', '', item))); details.append(element('h4', '', '写作要求'), req); const outline = element('ul', 'source-list'); question.reference_outline.forEach(item => outline.append(element('li', '', item))); details.append(element('h4', '', '参考提纲'), outline); practiceReader.append(details); addExplanation(question); const record = element('button', 'button secondary', '记录论文素材'); record.type = 'button'; record.addEventListener('click', () => { recordPractice(question, { type: '论文素材', result: '待复习', rule: form.elements.namedItem('solution').value.trim(), missedKeywords: form.elements.namedItem('body').value.trim(), errorType: '', }); field('project_code').value = project.slice(0, 120); field('decision').value = form.elements.namedItem('solution').value.trim(); field('tradeoff').value = form.elements.namedItem('tradeoff').value.trim(); field('outcome').value = form.elements.namedItem('outcome').value.trim(); }); practiceReader.append(record); }); practiceReader.append(form);
  }
  async function openPracticeQuestion(id) {
    practiceState.textContent = '正在加载已核对题目…';
    try { const response = await fetch(`/api/practice/questions/${encodeURIComponent(id)}`, { headers: { Accept: 'application/json' } }); const payload = await response.json(); if (!response.ok) throw new Error(payload.error || '题目加载失败'); const question = payload.question; activePracticeQuestion = question; practiceState.textContent = `正在练习：${question.title}`; if (question.type === 'single_choice') renderSingleChoice(question); else if (question.type === 'case_analysis') renderCaseAnalysis(question); else renderEssay(question); renderPracticeList(); }
