...
const workspace = fs.readFileSync(path.join(__dirname, '../site/workspaces/podcasts.js'), 'utf8');
assert.match(app, /function createPodcastController\(\)/);
assert.match(app, /return \{ load, list, selectChapter, toggle, snapshot, subscribe, openKnowledge, record \}/);
assert.match(app, /dialogs\.openStudyRecord/);
assert.match(workspace, /context\.podcast\.(?:load|subscribe|toggle|record)/);
assert.doesNotMatch(workspace, /createElement\(['"]audio['"]\)/);
